...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "revolver", "tests")); system.time(test_check("revolver", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran revolver_$(VERSION).tar.gz
	rm -rf revolver.Rcheck/
